...
# Parser to handle input arguments
...
# Clean the transcript to only include the allowed characters
def _preprocess_transcript(phrase):
    cleaned = ''.join([ch for ch in phrase if ch in 'abcdefghijklmnopqrstuvwxyz '])
...
# Function to create the CSV with valid transcript
def create_csv_with_transcripts(split_dir, csv_filename):
    csv_path = os.path.join(split_dir, csv_filename)
    manifest_file = open(csv_path, "w", newline="")
    csv_writer = csv.writer(manifest_file)
    for root, subdirs, files in tqdm(os.walk(split_dir)):
        for f in files:
            if f.endswith(".wav"):
                txt_file_path = os.path.join(root, f.replace(".wav", ".txt"))
                if os.path.exists(txt_file_path):
                    with open(txt_file_path, "r") as txt_file:
                        transcript = txt_file.read().strip()
                        # Skip empty transcripts
                        if transcript:
                            # Ensure valid characters in transcript
                            valid_transcript = _preprocess_transcript(transcript)
                            if valid_transcript:  # Skip if still empty after cleaning
                                # Only write the transcript, not the file location
                                csv_writer.writerow([valid_transcript])  # Write only the transcript
    manifest_file.close()
...
def main():
    ...
    for split_type, lst_libri_urls in LIBRI_SPEECH_URLS.items():
        ...
        for url in lst_libri_urls:
            ...
            shutil.rmtree(extracted_dir)
        # After processing, create the CSV for this split
        create_csv_with_transcripts(split_dir, f"libri_{split_type}_manifest.csv")
